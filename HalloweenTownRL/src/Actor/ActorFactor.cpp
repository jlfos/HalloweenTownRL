...
Actor *ActorFactory::CreateHero(int x, int y){
	try{
		...
		player->destructible = std::shared_ptr<PlayerDestructible>(new PlayerDestructible(30,2,"your cadaver"));
		player->attacker = std::shared_ptr<Attacker>(new Attacker(3, "fists"));
		player->ai = std::shared_ptr<PlayerAi>(new PlayerAi());
		player->container = std::shared_ptr<Container>(new Container(26));
		player->lightsource = std::shared_ptr<Lightsource>(new Lightsource(30));
		return player;
	}
	catch(...){
...
Actor *ActorFactory::CreateLampPost(int x, int y){
	try{
		...
		lampPost->lightsource = std::shared_ptr<Lightsource>(new Lightsource(15));
		return lampPost;
	}
	catch(...){
...
Actor *ActorFactory::CreateGiantSpider(int x, int y){
	try{
		...
		giantSpider->destructible = std::shared_ptr<BossDestructible>(new BossDestructible(120, 6, 0,"dead giant spider"));
		giantSpider->attacker = std::shared_ptr<Attacker>(new Attacker(20));
		giantSpider->ai = std::shared_ptr<MonsterAi>(new MonsterAi(70));
		return giantSpider;
	}
	catch(...){
...
Actor *ActorFactory::CreateVampire(int x, int y, EnemyDifficulty difficulty){
	try{
		...
		if(difficulty == EnemyDifficulty::MEDIUM){
			...
			vampire->destructible = std::shared_ptr<MonsterDestructible>(new MonsterDestructible(15, 1, 6, "pile of ash"));
			vampire->attacker = std::shared_ptr<Attacker>(new Attacker(3));
			vampire->ai = std::shared_ptr<MonsterAi>(new MonsterAi(35));
		...
		if(difficulty == EnemyDifficulty::HARD){
			...
			vampire->destructible = std::shared_ptr<MonsterDestructible>( new MonsterDestructible(30, 2, 12, "pile of ash"));
			vampire->attacker = std::shared_ptr<Attacker>( new Attacker(4));
			vampire->ai = std::shared_ptr<MonsterAi>( new MonsterAi(35));
			...
			vampire->destructible = std::shared_ptr<MonsterDestructible>( new MonsterDestructible(45, 3, 24, "pile of ash"));
			vampire->attacker = std::shared_ptr<Attacker>( new Attacker(8));
			vampire->ai = std::shared_ptr<MonsterAi>( new MonsterAi(35));
		}
			...
			vampire->destructible = std::shared_ptr<MonsterDestructible>( new MonsterDestructible(80, 4, 48, "pile of ash"));
			vampire->attacker = std::shared_ptr<Attacker>( new Attacker(16));
			vampire->ai = std::shared_ptr<MonsterAi>( new MonsterAi(45));
		}
			...
			vampire->destructible = std::shared_ptr<MonsterDestructible>( new MonsterDestructible(120, 6, 96, "pile of ash"));
			vampire->attacker = std::shared_ptr<Attacker>( new Attacker(20));
			vampire->ai = std::shared_ptr<MonsterAi>( new MonsterAi(70));
		}
		return vampire;
	}
	catch(...){
...
Actor *ActorFactory::CreateGremlin(int x, int y){
	try{
		Actor *gremlin = new Actor( x, y, TileCharacters::Default::G_LOWERCASE, "gremlin",
				TCODColor::desaturatedGreen);
		gremlin->destructible = std::shared_ptr<MonsterDestructible>( new MonsterDestructible(6,0, 3,"dead gremlin"));
		gremlin->attacker = std::shared_ptr<Attacker>( new Attacker(1));
		gremlin->ai = std::shared_ptr<MonsterAi>( new MonsterAi());
		return gremlin;
	}
	catch(...){
...
Actor *ActorFactory::CreateZombie(int x, int y){
	try{
		...
		zombie->destructible = std::shared_ptr<MonsterDestructible>( new MonsterDestructible(4, 0, 1, "dead zombie"));
		zombie->attacker = std::shared_ptr<Attacker>( new Attacker(3));
		zombie->ai = std::shared_ptr<MonsterAi>( new MonsterAi(10));
		return zombie;
	}
	catch(...){
...
Actor *ActorFactory::CreateMedkit(int x, int y){
	try{
		...
		healthPotion->item = std::shared_ptr<Healer>( new Healer(4));
		return healthPotion;
	}
	catch(...){
...
Actor *ActorFactory::CreateSewerMutant(int x, int y){
	try{
		...
		mutant->destructible = std::shared_ptr<MonsterDestructible>( new MonsterDestructible(25, 1, 0, "dead mutant"));
		mutant->attacker = std::shared_ptr<Attacker>( new Attacker(4));
		mutant->ai = std::shared_ptr<MonsterAi>( new MonsterAi());
		return mutant;
	}
	catch(...){
...
Actor *ActorFactory::CreateGoblin(int x, int y){
	try{
		...
		goblin->destructible = std::shared_ptr<MonsterDestructible>( new MonsterDestructible(12, 1, 2, "dead goblin"));
		goblin->attacker = std::shared_ptr<Attacker>( new Attacker(3));
		goblin->ai = std::shared_ptr<MonsterAi>( new MonsterAi());
		return goblin;
	}
	catch(...){
...
Actor *ActorFactory::CreateDirewolf(int x, int y){
	try{
		...
		direWolf->destructible = std::shared_ptr<MonsterDestructible>( new MonsterDestructible(7, 0, 1, "dead wolf"));
		direWolf->attacker = std::shared_ptr<Attacker>( new Attacker(2));
		direWolf->ai = std::shared_ptr<MonsterAi>( new MonsterAi(40));
		return direWolf;
	}
	catch(...){
...
Actor *ActorFactory::CreateDirebear(int x, int y){
	try{
		...
		direBear->destructible = std::shared_ptr<MonsterDestructible>( new MonsterDestructible(80, 0, 6, "dead bear"));
		direBear->attacker = std::shared_ptr<Attacker>( new Attacker(15));
		direBear->ai = std::shared_ptr<MonsterAi>( new MonsterAi(40));
		return direBear;
	}
	catch(...){
...
Actor *ActorFactory::CreateGiantInsect(int x, int y){
	try{
		...
		giantInsect->destructible = std::shared_ptr<MonsterDestructible>( new MonsterDestructible(40, 4, 24, "dead insect"));
		giantInsect->attacker = std::shared_ptr<Attacker>( new Attacker(9));
		giantInsect->ai = std::shared_ptr<MonsterAi>( new MonsterAi());
		return giantInsect;
	}
	catch(...){
...
Actor *ActorFactory::CreateNightstick(int x, int y){
	try{
		...
		nightstick->item = std::shared_ptr<MeleeWeapon>( new MeleeWeapon(9));
		return nightstick;
	}
	catch(...){
...
Actor *ActorFactory::CreateTireIron(int x, int y){
	try{
		...
		tireIron->item = std::shared_ptr<MeleeWeapon>( new MeleeWeapon(18));
		return tireIron;
	}
	catch(...){
...
Actor *ActorFactory::CreateKnife(int x, int y){
	try{
		Actor *knife = new Actor(x, y, TileCharacters::Default::FORWARD_SLASH, "knife",
				TileColors::lighterGrey);
		knife->blocks = false;
		knife->item = std::shared_ptr<MeleeWeapon>( new MeleeWeapon(7));
		return knife;
	}
	catch(...){
...
Actor *ActorFactory::CreateKris(int x, int y){
	try{
		Actor *kris = new Actor(x, y, TileCharacters::Default::FORWARD_SLASH, "kris",
				TileColors::grey);
		kris->blocks = false;
		kris->item = std::shared_ptr<MeleeWeapon>( new MeleeWeapon(22));
		return kris;
	}
	catch(...){
		std::cerr << "An error occurred in ActorFactory::CreateNightStick" << std::endl;
...
Actor *ActorFactory::CreateBaseballBat(int x, int y){
	try{
		...
		baseballBat->item = std::shared_ptr<MeleeWeapon>( new MeleeWeapon(36));
		return baseballBat;
	}
	catch(...){
...
Actor *ActorFactory::CreateMachete(int x, int y){
	try{
		Actor *machete = new Actor(x, y, TileCharacters::Default::AT_SIGN, "machete", TileColors::white);
		machete->blocks = false;
		machete->item = std::shared_ptr<MeleeWeapon>( new MeleeWeapon(38));
		return machete;
	}
	catch(...){
